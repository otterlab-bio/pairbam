...
Mate preservation is measured, not asserted. Four cells run against one immutable revision of
`fallingstar10/otter-data`, each verified against the published checksum manifest, with exact
input/output record counts enforced:

- the BS-PDX graft cell passes **525,922** records through unchanged;
- the BS-PDX host cell passes **208,812**;
- the RNA-PDX cell carries supplementary alignments, so it exercises the drop-non-primary path and
  publishes exactly **40,000** records from 20,000 fragments;
- a seeded negative control perturbs 7 read-name groups to an odd record count and asserts that
  precisely those 7 are rejected and no others.

The [Gate B section](#gate-b-reproducibility-check) below carries the per-cell table and the exact
acceptance contract.
...
```bash
git clone https://github.com/otterlab-bio/pairbam.git
cd pairbam
go build -o pairbam ./cmd/pairbam
```

`pairbam` uses the shared [`bamdriver`](https://github.com/otterlab-bio/bamdriver) module for low-level BAM/BGZF operations.
...
MIT · [otterlab-bio/pairbam](https://github.com/otterlab-bio/pairbam)
